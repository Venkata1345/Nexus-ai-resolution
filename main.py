...
# load_dotenv() must run before the imports below, because src.agents.agent
# instantiates the Gemini client at module-load time and reads GEMINI_API_KEY.
load_dotenv()
...
def run_nexus():
    ...
    for _ in nexus_app.stream(initial_input, config, stream_mode="values"):
        pass
    ...
    if "billing_worker" in next_step:
        ...
        if approval.lower() == "approve":
            ...
            for _ in nexus_app.stream(None, config, stream_mode="values"):
                pass
